...
fn print_help() {
    ...
    println!(
        "  aitrium-radiotherapy-server margin --rtstruct <RS.dcm> --from <name> --to <name> [--direction <dir>] [options]"
    );
    println!(
        "    clearance sign: positive = source inside target with margin, negative = protrusion"
    );
